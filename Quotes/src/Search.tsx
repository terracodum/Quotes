...
function Search(prop: Props) {
  const [currentCategory, setCurrentCategory] = useState(prop.category);
    ...
    return fetch(API + "authors", { method: "GET" })
      ...
      .then((list) => {
        for (let i = 0; i < list.length; i++) {
          resAuthors.push(
            new Author(list[i].slug, list[i].name, list[i].quoteCount),
          );
        }
        return resAuthors;
      });
  ...
  useEffect(() => {
    FetchAuthors().then((data: any) => {
      console.log(data);
      setAuthors(data);
    });
    FetchTags().then((data: any) => {
      console.log(data);
      setTags(data);
    });
  }, [currentCategory]);
  ...
  function ToTag(tag: TagCls) {
    return (
      <Tag id={tag.Id} name={tag.Name} key={tag.Id} count={"-1"} type="tag" />
    );
  }
